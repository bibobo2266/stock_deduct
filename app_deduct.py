...
# 用關鍵字比對而不是列舉產業字串，因為 FinMind 上市/上櫃的 industry_category
# 寫法不一致（「金融保險」/「金融業」/「金融保險業」都出現過）。
LOW_VOL_KEYWORDS = ("金融", "銀行", "保險", "證券", "電器電纜", "水泥")
...
def is_low_vol(industry: str) -> bool:
    ...
    if not industry:
        return False
    return any(k in str(industry) for k in LOW_VOL_KEYWORDS)
...
def tag_cluster(df: pd.DataFrame, min_n: int = 3) -> pd.DataFrame:
    ...
    寫死只擋金融的話其他族群照樣塞名額。"""
    out = df.copy()
    cnt = out["產業"].replace("", np.nan).value_counts()
    out["_同業家數"] = out["產業"].map(cnt).fillna(1).astype(int)
    out["_族群密集"] = out["_同業家數"] >= min_n
    ...
    for _, r in out.iterrows():
        ...
        if r["_族群密集"]:
            n.append(f"{r['產業']}同時上榜 {r['_同業家數']} 檔，高度連動，視為同一個部位")
        notes.append("；".join(n))
    out["註記"] = notes
    return out
...
if summaries:
    ...
    if demote and not hit.empty:
        hit = (hit.assign(_d=(hit["_族群密集"] | hit["_低波動"]).astype(int))
                  .sort_values("_d", kind="stable")
                  .drop(columns="_d"))
    _hidden = ["解讀", "_同業家數", "_族群密集", "_低波動"]
