...
def summarize(bars: pd.DataFrame, tbl: pd.DataFrame, ma_series,
    ...
    tol = 0.03   # 實測 120 檔 W20：±5pp 會標到 34%，±3pp 降到 23%，旗標才有鑑別度
    edge = any(_grid(width + dw, pos + dp) != state
               for dw in (-tol, 0, tol) for dp in (-tol, 0, tol))
...
if summaries:
    ...
    with summary_slot:
        ...
        # 收斂期數已改為排序鍵，不再是篩選條件（實測對窄箱組沒有鑑別度）
