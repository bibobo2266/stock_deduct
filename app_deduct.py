...
v7.2 — 加入箱寬百分位。九宮格的 30%/60% 是絕對門檻，分不出「在收縮」與
       「本來就不動」；百分位是拿這檔跟它自己比，≤20% 才算真的在縮。

v7.1 — 加入族群辨識三欄（產業 / 族群 / 註記）。
     動機：金融股天生低波動，箱寬 12% 對它是日常、對台積電是極度收縮，
     絕對門檻會把整族群低波動股一起放進來佔名額，而且它們同進同出，
     命中五檔金控實際上只是一個部位。這裡不篩掉，只標示 + 預設排到最後。
"""
import io
...
# 天生低波動的產業：箱寬窄不代表在收縮，是常態。
# 用精確類別而不是關鍵字——關鍵字「證券」會誤抓「所有證券」「受益證券」
# 「指數投資證券(ETN)」這些不是券商股的類別（實測 64 檔誤判）。
LOW_VOL_EXACT = {"金融保險", "金融業", "電器電纜", "水泥工業"}

# ETF / ETN / 受益憑證：不是個股，箱寬與族群邏輯都不適用。
# 同類別動輒 268 檔，放進族群計數會變成沒意義的「同業 268 檔」。
NON_STOCK_KEYWORDS = ("ETF", "ETN", "受益證券", "指數投資證券", "所有證券",
                      "指數股票型基金")

# 族群計數用的類別正規化。FinMind 把同一個族群按上市/上櫃拆成兩個字串
# （金融保險 vs 金融業、電子通路業 vs 電子通路），不併的話族群會少算一半。
INDUSTRY_ALIAS = {
    "金融業": "金融保險",
    "電子通路": "電子通路業",
    "文化創意": "文化創意業",
...
def load_prices(as_of=None, years: int = YEARS_DEFAULT) -> pd.DataFrame:
    """讀還原股價，分年存檔只載需要的年份。
    as_of 不為 None 時截斷到該日，等同回到那天重跑一次——扣抵值不產生
    買賣訊號，所以不需要預先算好的歷史表，單檔即時重算不到 1 秒。"""
    ...
    df = df[df["close"] > 0]
    # 還原股價母體含 ETF、興櫃、下市股，四碼開頭非 0 才是普通股
...
def is_low_vol(industry: str) -> bool:
    """該產業是否天生低波動。低波動 ≠ 收縮，箱寬要跟同業比才有意義。"""
    return str(industry).strip() in LOW_VOL_EXACT


def canon_industry(industry: str) -> str:
    """族群計數用的正規化名稱。顯示仍用原始字串，不動使用者看到的欄位。"""
    k = str(industry).strip()
    return INDUSTRY_ALIAS.get(k, k)


def is_non_stock(industry: str) -> bool:
    """ETF / ETN / 受益憑證。它們不是個股，不進族群計數。"""
    if not industry:
        return False
    return any(k in str(industry) for k in NON_STOCK_KEYWORDS)


def tag_cluster(df: pd.DataFrame, min_n: int = 3) -> pd.DataFrame:
    """在「本次上榜名單」內數同產業家數，達門檻就標族群旗標。
    刻意動態算而不是寫死擋金融——塑化、營建、航運同樣會整族群橫盤，
    寫死只擋金融的話其他族群照樣塞名額。"""
    out = df.copy()
    out["_非個股"] = out["產業"].map(is_non_stock)
    # ETF/ETN 不進族群計數，否則「同業 268 檔」會淹掉真正有意義的族群訊號
    out["_族群名"] = out["產業"].map(canon_industry)
    stock_only = out.loc[~out["_非個股"], "_族群名"].replace("", np.nan)
    cnt = stock_only.value_counts()
    out["_同業家數"] = out["_族群名"].map(cnt).fillna(1).astype(int)
    out["_族群密集"] = (out["_同業家數"] >= min_n) & (~out["_非個股"])
    out["_低波動"] = out["產業"].map(is_low_vol)
    out["族群"] = np.where(
        out["_族群密集"],
        "⚠️ 同業 " + out["_同業家數"].astype(str) + " 檔",
        "",
    )
    notes = []
    for _, r in out.iterrows():
        n = []
        if r["_非個股"]:
            n.append("ETF / ETN，非個股，箱寬與族群邏輯不適用")
        if r["_低波動"]:
            n.append("天生低波動，箱寬需與同業比較")
        if r["_族群密集"]:
            n.append(f"{r['_族群名']}同時上榜 {r['_同業家數']} 檔，高度連動，視為同一個部位")
        notes.append("；".join(n))
    out["註記"] = notes
    return out
...
def summarize(bars: pd.DataFrame, tbl: pd.DataFrame, ma_series,
    ...
    # 箱寬百分位：當前箱寬在「這一檔自己」的歷史箱寬分佈中的位置。
    # 九宮格的 30% / 60% 是絕對門檻，分不出「在收縮」和「本來就不動」——
    # 中華電箱寬 8%、台積電 16%，絕對值兩個都是窄箱，
    # 但中華電在自身歷史是第 74 百分位（比平常還寬），台積電是第 10（真的在縮）。
    # 樣本用目前載入的全部 bars（受側欄年數設定影響），日線週線都適用。
    rh = bars["high"].rolling(look).max()
    rl = bars["low"].rolling(look).min()
    hist = ((rh - rl) / rl).dropna()
    pctile = float((hist <= width).mean()) if len(hist) >= look * 4 else np.nan
    ...
    return {
        ...
        "箱體寬度": width,
        "箱寬百分位": pctile,
        "在收縮": bool(pctile <= 0.20) if pctile == pctile else False,
        "箱內位置": pos,
        "狀態": state,
        "邊界": edge,
        "箱頂": round(box_hi, 2),
...
def explain(r) -> str:
    ...
    _pc = r.get("箱寬百分位", np.nan)
    if _pc == _pc:
        p.append(f"箱寬 {b:.0%} 在自身歷史的第 {_pc:.0%} 百分位 → "
                 + ("真的在收縮" if _pc <= 0.20 else
                    ("偏縮" if _pc <= 0.40 else "不是收縮，是它的常態")))
    if st_ == "窄箱貼頂":
...
with st.sidebar:
    ...
    cluster_n = st.slider(
        "同產業幾檔以上算族群", 2, 8, 3,
        help="同一產業在本次名單中達到這個家數就標 ⚠️。它們通常同進同出，"
             "命中五檔金控實際上只是一個部位。",
    )
    demote = st.checkbox(
        "族群 / 低波動排到最後", value=True,
        help="不篩掉，只把它們壓到表格下方，掃表時不佔前排視線。",
    )
...
if summaries:
    ...
    # 族群旗標在「篩選後的名單」上算，不是在全母體上算——
    # 要回答的是「這批結果裡有幾檔是同一個族群」。
    hit = tag_cluster(hit, cluster_n)
    if demote and not hit.empty:
        hit = (hit.assign(_d=(hit["_族群密集"] | hit["_低波動"]
                              | hit["_非個股"]).astype(int))
                  .sort_values("_d", kind="stable")
                  .drop(columns="_d"))
    _hidden = ["解讀", "_同業家數", "_族群密集", "_低波動", "_非個股", "_族群名"]
    # 欄位順序：代號 名稱 產業 族群 …數字… 註記 在最後
    _order = [c for c in hit.columns if c not in _hidden + ["註記"]] + ["註記"]
    ...
    with summary_slot:
        ...
        if hit.empty:
            ...
            _lv = int(hit["_低波動"].sum())
            _cl = int(hit["_族群密集"].sum())
            if _lv or _cl:
                st.caption(
                    f"本批 {len(hit)} 檔中，{_lv} 檔屬天生低波動產業、{_cl} 檔落在族群密集區"
                    + ("（已排到表格下方）" if demote else "")
                    + "。低波動族群的窄箱是常態不是收縮訊號，箱寬請跟同業比。"
                )
            st.dataframe(
                hit[_order],
                ...
                column_config={
                    ...
                    "箱寬百分位": st.column_config.NumberColumn(
                        format="%.0f%%",
                        help="當前箱寬在這檔自己歷史箱寬分佈中的位置。"
                             "≤20% 才是真的在收縮；高百分位的窄箱只是天生窄。"),
                    "在收縮": st.column_config.CheckboxColumn(
                        help="箱寬百分位 ≤ 20%。這是相對自己，不是相對別檔。"),
                    "收斂期數": st.column_config.NumberColumn(format="%.0f"),
                    "族群": st.column_config.TextColumn(width="small",
                        help="本次名單中同產業家數達門檻。這些股票高度連動，算一個部位。"),
                    "註記": st.column_config.TextColumn(width="large",
                        help="低波動 / 族群密集的提醒。只是標示，沒有篩掉任何東西。"),
                },
            )
            ...
            for _, r in hit.head(40).iterrows():
                _mark = (" " + r["族群"]) if r["族群"] else ""
                with st.expander(f"{r['代號']} {r['名稱']} {r['現價']} "
                                 f"乖離 {r['乖離率']:.1%}{_mark}"):
                    if r["註記"]:
                        st.warning(r["註記"])
        ...
        st.download_button(
            "⬇️ 下載摘要 (CSV)",
            data=(tag_cluster(sdf, cluster_n)
                  .drop(columns=["_同業家數", "_族群密集", "_低波動", "_非個股", "_族群名"])
